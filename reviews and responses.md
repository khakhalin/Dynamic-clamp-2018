...
Here, the actual rationale for the second explanation (a difference in subcellular targets) is that inputs to different parts of the dentritic tree can trigger action potentials of different shape, and differentially affect calcium dynamics in the neuron, which, together with non-linear processing in dendrites may make different types of inputs unequal in terms of their effects on neuronal activity, and neuronal tuning, respectively. Two extreme examples of this effect would be the impact of climbing fibers on the activity of Purkinje cells in the cerebellum, and the impact of inputs to the apical dendrites of pyramidal cells in the cortex. We feel, however, that this topic is very complex, yet somewhat tangential to the main message of the paper, so we tried to limit our exposition into it by a short sentence and a good, recent reference.

SILAS-SAYS: For the climbing fiber to purkinje cell example, it may be worth noting that the climbing fiber makes hundreds of synapses along pretty much all of the primary dendritic arbor of the purkinje cell. So It doesn't really represent a spatially specific input to the dendritic arbor, except that there aren't really as many CF inputs to the distal most dendrites, so it is slightly proximal leaning. Also, I'm reading a paper now by Davie, Clark, and Hausser 2008 where they use DC to show that CF mediated dendritic spikes don't actually influence the axosomatic generated complex spike. Instead the huge Ca influx from CF input mediates LTD at Parallel fiber-purkinje cell synapses, so it is absolutely a great example to show the importance of calcium dynamics mediated by dendritic input on plasticity.
...
In the originally submitted pdf, lines 2 to 5 read:
...
SILAS-SAYS: While I think you answered the question, I don't think R2 will like the change. I think they want us to write in the text something like this: "As previous studies show that synaptic responses to flashes are much shorter than responses to looming stimuli in the tectum (Khakhalin 2014, Khakhalin 2019), and changes in intrinsic properties seemed homeostatic (ie increased activation led to reduced spiking), one could expect shorter stimuli (flashes) to selectively suppress responses to shorter synaptic inputs." This uses your response to the reviewer to write what may be a clearer version of that particular sentence. When I get to the manuscript I will make a note about this comment at that place.
...
SILAS-SAYS: My suggestion for this sentence, in the comment above, does not help to elaborate this point that R2 brings up, but I'm also not sure if your rewrite is resolving the confusion of R2. Maybe we just need an additional sentence before the problem sentence saying something along these lines: "One could expect that, if activity can induce a homeostatic suppression of excitability, then a specific profile of activity, such as a short duration, may cause a calibrated homeostatic suppression of a cell's excitability to that specific input profile."
...
We hope that the word "prefer" is intuitive, and it has a benefit of being way shorter than "responded with more spikes", or "produced more action potentials", which hopefully makes reading easier. Realistically, we hope that most readers of this article will not be at risk of truly personifying neurons, so while we take care to never personify animals, we are intentionally a bit more relaxed when writing about cells, channels, receptors etc. Still, to make this sentence better, we removed the quotes around "preferred", to make it more obvious that it is not an antropomorphic allegory, but just a normal word use. Overall, we use the word "prefer" 6 times in the paper, always in the same way: when comparing spiking responses to different stimuli, to describe the stimulus that produced sronger outputs in a given set of neurons.

SILAS-SAYS: I'm kinda with the reviewer on this one. I agree that it's nice to use a potentially more intuitive word that shortens things, but at the very least I think we should explicitely state that prefered here means spiked more ("While control neurons produced increasing spikes in response to increasing input durations, exhibiting a preference for long synaptic inputs, neurons exposed to flashes..." ). Another issue R2 may be feeling but not saying explicitely is that in saying prefered, we seem to be suggesting a specific hypothesis about the goal of intrinsic plasticity of intrinsic excitability (ie retuning contributes something specific to the intake and export of information in a tectal neuron). Of course, we think that it is very important, but we don't have follow up experiments displaying what specific affects tectal retuning could have on sensory processing.
...
SILAS-SAYS: While you've certainly explained our approach to R2, I wonder if R2 will want us to make the same explicit statements about what each term mean in the manuscript itself.
...
We did not measure the resting membrane potential directly, but measured the holding current necessary to bring the cell to -60 mV in voltage clamp mode, and the mean value of this I_hold value was -8.2±4.8 pA, which is similar (most importantly, not higher) than what was previously reported in the literature (-19.8±16.6 in Ciearleglio Khakhalin 2015). 
...
Since the mistake that you noticed (thank you again for being so thorough!) was a typo in the text, and not a mistake in the data, it does not affect any calculations reported in this paper.
...
SILAS-SAYS: It seems R2 is asking if most cells follow the pattern of spike increase with duration increase as highlighted by the mean, or if the sum of many different activation patterns just happens to generate that average. I think that your point about the correlation coefficient puts that to rest, but you may want to say that more explicitely because it is legitimately difficult to look at the individual data points to this level of precision without seeing the mean trend.
